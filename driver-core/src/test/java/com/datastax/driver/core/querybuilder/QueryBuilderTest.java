...
public class QueryBuilderTest {
    ...
    public void insertTest() throws Exception {
        ...
        // if not exists
        query = "INSERT INTO foo.bar(a,b) VALUES (123,456) IF NOT EXISTS;";
        insert = insertInto("foo", "bar")
                .ifNotExists()
                .value("a", 123)
                .value("b", 456);
        ...
        try {
            insertInto("foo").ifNotExists().value("a", 123).value("b", 456).using(timestamp(123)).toString();
            fail();
        } catch (IllegalStateException e) {
            assertEquals(e.getMessage(), "ifNotExists CAS operation is not compatible with options.");
        }
